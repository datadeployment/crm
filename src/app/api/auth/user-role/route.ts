...
import qs from 'qs';
import { create_user_role_table_query } from "./constant";
    ...
    try {
        ...
        if (res_data.status_code === 200) {
            ...
            if (id) {
                ...
                id as value,
                name AS label
                FROM UserRole ${whereClauseQuery} ${searchQuery} ${sortQuery} LIMIT ? OFFSET ?`
                ...
                const get_user_count: any = await query({
                    query: `SELECT COUNT(*) as count FROM UserRole`,
                    values: [],
                });
